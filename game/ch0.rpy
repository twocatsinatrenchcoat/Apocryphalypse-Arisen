...
define player = Character("",what_color=persistent.bloodclr,what_prefix="You: ",kind=nvl)
...
label ch0:
    ...
    pchum "{color=#000}--- {color=#a30000}apocalypseArisen{/color} started pestering{/color} You {color=#000}at 15:37 ---{/color}"
    ...
    menu (nvl=True):
        ...
        "Pack my box with...":
            python:
                renpy.say(player, "{cps=15}"+quirk("Pack my box with five dozen liquor jugs.{/cps}")+"{/cps}")
